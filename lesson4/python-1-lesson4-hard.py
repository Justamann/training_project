...
def withdraw_money(person, money):
    if person['money'] - money >= 0 and money == abs(money):
        person['money'] -= money
...
def start():
    try:
        ...
        if re.search(check_card_number_and_pin, card_number):
            card_number = int(card_number)
        if re.search(check_card_number_and_pin, pin_code):
            pin_code = int(pin_code)
    except ValueError:
        print('Ancorrect data')
